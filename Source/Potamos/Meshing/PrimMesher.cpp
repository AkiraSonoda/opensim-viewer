...
void AngleList::makeAngles(int sides, float startAngle, float stopAngle) {
	...
	if ((sides == 3 || sides == 4 || sides == 24)) {
		...
		if (startAngle > angles[0].angle) {
			Angle newAngle;
			intersection(angles[0].X, angles[0].Y, angles[1].X, angles[1].Y, 0.0f, 0.0f, (float)cos(startAngle),
			             (float)sin(startAngle));
		...
		if (stopAngle < angles[index].angle) {
			...
			intersection(angles[index - 1].X, angles[index - 1].Y, angles[index].X, angles[index].Y, 0.0f, 0.0f,
			             (float)cos(stopAngle), (float)sin(stopAngle));
...
Profile::Profile(int sides, float profileStart, float profileEnd, float hollow, int hollowSides, bool createFaces,
                 bool calcVertexNormalsIn) {
	numOuterVerts = 0;
	numHollowVerts = 0;
	outerFaceNumber = -1;
	hollowFaceNumber = -1;
		...
		if (createFaces) {
			...
			if (numOuterVerts == numHollowVerts) {
				...
				if (numOuterVerts < numHollowVerts) {
					...
					{
						if (j < maxJ)
							if (angles.angles[j + 1].angle - hollowAngles.angles[i].angle < hollowAngles.angles[i].angle
								- angles.angles[j].angle + 0.000001f) {
				...
				} else // numHollowVerts < numOuterVerts
				{
					...
					for (int i = 0; i < numOuterVerts; i++) {
						if (j < maxJ)
							if (hollowAngles.angles[j + 1].angle - angles.angles[i].angle < angles.angles[i].angle -
								hollowAngles.angles[j].angle + 0.000001f) {
								newFace.v1 = i;
