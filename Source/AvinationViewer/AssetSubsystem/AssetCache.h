...
public:
	AssetDecodeThread(AssetCache* c) {
		cache = c;
		thread = FRunnableThread::Create(this, TEXT("AssetDecode"), 0, TPri_BelowNormal);
	}

	virtual ~AssetDecodeThread() {
		Stop();
...
public:
	AssetProcessThread(AssetCache* c) {
		cache = c;
		thread = FRunnableThread::Create(this, TEXT("AssetDecode"), 0, TPri_BelowNormal);
	}

	virtual ~AssetProcessThread() {
		Stop();
		thread->WaitForCompletion();
		delete thread;
	}
