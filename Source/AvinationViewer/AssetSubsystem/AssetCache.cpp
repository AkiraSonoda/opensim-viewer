...
void AssetCache::Fetch(FGuid id, AssetFetchedDelegate delegate) {
	...
	if (!queueIndex.Contains(id)) {
		...
		if (IFileManager::Get().FileExists(*ppath)) {
			TSharedAssetFetchContainerRef cachedAsset = MakeShareable(
				new AssetFetchContainer(id, MakeShareable(new T())));
...
void AssetCache::RequestDone(FHttpRequestPtr request, FHttpResponsePtr response, bool bWasSuccessful,
                             TSharedAssetFetchContainerRef container) {
	FScopeLock l(&queueLock);
